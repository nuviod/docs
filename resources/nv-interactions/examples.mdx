...
description: Production-ready MDX documentation for nv-interactions. Covers API usage, parameters, return values, state management, and real-world examples.
mode: wide
----------

## Overview

`nv-interactions` provides a lightweight system for registering context-based interactions that players can trigger using a key press. Interactions can be attached to **world coordinates** or **entities** and can expose one or more selectable options.

This documentation is fully **MDX-compatible** and intended for Mintlify.
...
* Always store the returned `groupKey` if the interaction needs to be controlled later
...
* Keep interaction logic lightweight and event-driven

---

## Notes

* Interactions are cached and reused for performance
* Invalid entity IDs or coordinates will silently fail
* Performance scales linearly with active interaction groups
